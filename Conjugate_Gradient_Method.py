...
def gradf(x):
    return np.array([(12*x[0] - 6*x[1] + 4 *x[2] +8), (-6*x[0] + 12* x[1] - 8 *x[2] + 2) , (4 * x[0] - 8*x[1] + 6*x[2] -4) ])
...
# Initialization
x = np.array([-4.5, -3.5, -1])  # Starting point
...
while k < Nmax:
    ...
    # Print iteration details, including sk
    print(f"Iteration {k}: x = {np.array2string(x, precision=10, suppress_small=True)}, f(x) = {f(x):.10f}, sk = {sk:.10f}, Gradient Norm = {np.linalg.norm(grad):.10f}")

if k == Nmax:
    print("Maximum iteration limit reached.")

# Final root output
print(f"Final x_root: {np.array2string(x, precision=10, suppress_small=True)}")
